                    ...
                    <form method="POST" action="{{ route('events.update', $event) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" rows="5"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 @error('description') border-red-500 @enderror">{{ old('description', $event->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="facility_id" id="facility_id"
                                ...
                                @foreach(\App\Models\Facility::where('user_id', auth()->id())->get() as $facility)
                                    <option value="{{ $facility->id }}" {{ old('facility_id', $event->facility_id) == $facility->id ? 'selected' : '' }}>
                                        {{ $facility->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="flex items-center justify-end gap-4">
                            <a href="{{ route('events.show', $event) }}" class="text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200">
                                Anuluj
                            </a>
